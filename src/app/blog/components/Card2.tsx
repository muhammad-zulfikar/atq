...
const Card2: FC<Card2Props> = ({ className = "", post }) => {
  const { title, href, desc, featuredImage, date, postType } = post;

  return (
    <div className={`nc-Card13 relative flex ${className}`} data-nc-id="Card2">
      ...
      >
        <Image
          fill
          className="object-cover rounded-xl sm:rounded-3xl transition-transform duration-300 ease-in-out transform group-hover:scale-105"
          src={featuredImage}
          alt={title}
